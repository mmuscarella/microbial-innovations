ConsenTrait <- function(tree = "", traits = "", cutoff = 0.9, output = ""){
  ...
  #tree_all = read.tree(tree,keep.multi = TRUE)
  tree_all = tree
  #Loading trait table w. no headers
  #table = read.table(traits, sep = "\t", header=FALSE)
  table = traits
  ...
  for (m in 1:length(tree_all)) {#loop through all trees
    ...
    for (j in 2:ncol(table)) {
      ...
      for (i in 1:length(subtree)){
        ...
        if (mean(table2[tip_names][,Trait]) > cutoff ) {#change this value if you want a new threshold
  ...
  write.table(Mean_all,output, sep = "\t")
}
