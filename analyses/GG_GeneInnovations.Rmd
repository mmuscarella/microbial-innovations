---
title: "Microbial Innovations: GreenGenes Gene Innovations"
...
## Import Green Genes Tree and Taxonomy 
```{r}
GG <- read.tree("../data/IMG.tree")
GG.tax <- read.tax("../data/IMG.pds.wang.taxonomy", 
                   format = "rdp", col.tax = 2, tax.levels = 6)
```

## Rooted GreenGenes Tree
```{r, results = "hide"}
png(filename="../figures/GG_root.png",
    width = 1600, height = 1600, res = 96*2)

GG.tax2 <- GG.tax[match(GG$tip.label, GG.tax$OTU), ]
groupInfo <- split(GG$tip.label, GG.tax2$Domain)
GreenTree <- groupOTU(GG, groupInfo, group_name = "Domain")
...
# GreenGenes Traits: Gene Traits
## Import Trait and KEGG Databases
```{r}
...
# Core Genes (Found in All Organisms)
dim(IMG.traits)
core <- which(rowSums(IMG.traits > 1) >= (dim(IMG.traits)[2] * 0.9))
...
```{r}
IMG.Cons <- read.delim("../data/IMG_Genes_ConsenTrait.txt")

# Fetch Names from Trait Database
IMG.Cons$Name <- NA
for(i in 1:dim(IMG.Cons)[1]){
  temp <- IMG.Cons$trait[i]
  IMG.Cons$Name[i] <- rownames(IMG.traits)[temp]
}

# Create Gene Table
Gene.Cons <- as.data.frame(matrix(NA, ncol = 5, 
                           nrow = length(unique(IMG.Cons$Name))))
colnames(Gene.Cons) <- c("gene", "origins", "trait.mean", "trait.se", "nodes") 

# Node Correction (Con starts at 1 not at Ntips + 1)
Ntips <- length(GG.um$tip.label)
IMG.Cons$node2 <- as.numeric(IMG.Cons$node) + Ntips

# Populate Gene Table
Gene.Cons$gene <- sort(unique(IMG.Cons$Name))
for(i in 1:dim(Gene.Cons)[1]){
  gene <- Gene.Cons$gene[i]
  temp <- IMG.Cons[which(IMG.Cons$Name == gene), ]
  temp <- temp[which(temp$cluster_size > 1), ]
  nodes <- toString(temp$node2, collapse = ", ")
...
## ASR Predictions
```{r}
IMG.ASR.full <- read.delim("../data/IMG_genes_ASR_Long.txt")
...
for(i in 1:dim(Gene.ASR.full)[1]){
  ...
  Gene.ASR.full$trait.mean[i] <- mean(temp$R.Dist)
  Gene.ASR.full$trait.se[i] <- se(temp$R.Dist)
}
...
# Correct Dates (Subtract from Max Depth in Tree)
max.dist <- max(as.matrix(dist.nodes(GG.um))[,length(GG.um$tip.label) + 1])
...
par(mar=c(0,6,1,1))
barplot(xhist$counts[0:35], axes=FALSE, space=0, xlim = c(0, 35))
par(mar=c(5,0,1,1))
...
## Core Genes
```{r}
max(table(gene.dictionary$ko))


```


plot(Gene.ASR3$y, Gene.Cons3$trait.mean)

plot(log(-Gene.ASR3$x), log(-Gene.ASR3$y))
```
...
Nitrogen.Table <- gene.dictionary[which(gene.dictionary$C == N.pathway), ]
N.Genes <- Nitrogen.Table$ko

N.ASR <- Gene.ASR3[which(Gene.ASR3$gene %in% N.Genes), ]
N.Cons <- Gene.Cons3[which(Gene.Cons3$gene %in% N.Genes), ]

N.ASR$name <- NA
for(i in 1:dim(N.ASR)[1]){
  temp <- gene.dictionary$gene[which(gene.dictionary$ko == N.ASR$gene[i])]
...
plot(x = N.ASR$trait.mean, y = 1:dim(N.ASR)[1], 
     pch = 22, bg = "gray", cex = 3, type = "n",
     axes = F, xlab = "", ylab = "",
     xlim = c(4000, 0), ylim = c(0.5, dim(N.ASR)[1] + 0.5))
arrows(x0 = N.ASR$trait.mean, 
       x1 = N.ASR$trait.mean + N.ASR$trait.se, 
      y0 = 1:dim(N.ASR)[1], angle = 90, length = 0.05, lwd = 1.5)
arrows(x0 = N.ASR$trait.mean, 
       x1 = N.ASR$trait.mean - N.ASR$trait.se, 
      y0 = 1:dim(N.ASR)[1], angle = 90, length = 0.05, lwd = 1.5)
points(x = N.ASR$trait.mean, y = 1:dim(N.ASR)[1], 
     pch = 22, bg = "gray", cex = 3, lwd = 1.5)


arrows(x0 = N.Cons$trait.mean, 
       x1 = N.Cons$trait.mean + N.Cons$trait.se, 
      y0 = 1:dim(N.Cons)[1], 
...
arrows(x0 = N.Cons$trait.mean, 
      ...
      y0 = 1:dim(N.Cons)[1], 
      angle = 90, length = 0.1, lwd = 1.5, col = "red")
points(x = N.Cons$trait.mean, y = 1:dim(N.Cons)[1], 
     pch = 22, bg = "gray", cex = 1.5, lwd = 1.5, col = "red")

axis(side = 1, labels = T, lwd.ticks = 1.5)
axis(side = 2, labels = c(N.ASR$name), 
     at = 1:dim(N.ASR)[1], las = 1, lwd.ticks = 1.5)
axis(side = 4, labels = F, 
     at = 1:dim(N.ASR)[1], las = 1, lwd.ticks = 1.5)
axis(side = 2, labels = F, at = 1:dim(N.ASR)[1], 
     tck = 0.02, lwd.ticks = 1.5)
axis(side = 4, labels = F, at = 1:dim(N.ASR)[1], 
     tck = 0.02, lwd.ticks = 1.5)
...
plot(0, axes = F, xlab = "", ylab = "", type = "n",
     xlim = c(0, 1), ylim = c(0.5, dim(N.ASR)[1] + 0.5))
text(x = 0.5, y = 1:dim(N.ASR)[1]+0.2, 
     labels = as.character(round(-N.ASR$x, 6)), cex = 0.75)
text(x = 0.5, y = 1:dim(N.ASR)[1]-0.2, 
     labels = as.character(round(-N.ASR$y, 6)), cex = 0.75)
text(x = 0.5, y = dim(N.ASR)[1] + 0.75,
