...
# Some genes are in multiple pathways
#gene.table2[duplicated(gene.table2$ko), ]
```

## Import Trait Predictions
```{r}
...
## Trait Origin Summary
```{r}
length(subs)
subs.names <- rep(NA, length(subs))
for(i in 1:length(subs)){
  subs.names[i] <- subs[[i]]$name
}

# Identify Origins (First Occurence in Clade)
traits <- levels(IMG.ASR$Trait)
origins <- data.frame(matrix(NA, nrow = 1, ncol = dim(IMG.ASR)[2]))
colnames(origins) <- colnames(IMG.ASR)
counter = 1
for(i in traits){
  temp <- IMG.ASR[which(IMG.ASR$Trait == i), ]
  if(dim(temp)[1] > 1){
    print("yes")
    tip_names <- vector(mode="character", length=0)
    for(j in 1:dim(temp)[1]){
      temp_node <- which(subs.names == temp$Des[j])
      positives <- subs[[temp_node]]$tip.label
      if(all(is.na(match(tip_names, positives)))){
        tip_names <- c(tip_names, positives)
        origins[counter, ] <- temp[j, ]
        counter = counter + 1
      }
    }
  }
  if(dim(temp)[1] == 1){
    origins[counter, ] <- temp
    counter = counter + 1
  }
}
...
asr.mean <- mean(origins$R.Dist)
asr.se <- se(origins$R.Dist)

plot(IMG.ASR$R.Dist)

hist(IMG.ASR$R.Dist)
hist(IMG.Cons$distance)
...
## ConsenTrait Plot
```{r}
png(filename="../figures/IMG_TraitConservationDates.png",
...
## Show Plot
#```{r}
img <- readPNG("../figures/IMG_TraitConservationDates.png")
grid.raster(img)
```
...
## Show Plot
#```{r}
img <- readPNG("../figures/old_figs/GG_TraitASRDates.png")
grid.raster(img)
```
