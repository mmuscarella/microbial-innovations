...
JGI.full.tax <- JGI.meta[which(JGI.meta$IMG.Genome.ID %in% JGI.full.2$tip.label), ]
...
missing <- setdiff(JGI.full.2$tip.label,
                   JGI.full.tax$taxon_oid)
JGI.full.2 <- drop.tip(JGI.full.2, missing)
...
## Trim Genes for Only Those Genomes in Tree
```{r}
genes <- read.delim("../data/IMG.KEGG.txt", header = T)

JGItree.r$tip.label[1:5]

genes2 <- genes[genes$OID %in% JGItree.r$tip.label, ]

dim(genes)
dim(genes2)

# export trimmed genes file
write.table(genes2, "../data/IMG.KEGG.trimmed.txt", sep="\t") 
```


## JGI Exploratory Tree with Cyano Grouping
```{r, results = "hide"}
png(filename="../figures/JGI_Cyano.png",
    width = 1600, height = 1600, res = 96*2)

JGI.full.tax <- JGI.full.tax[order(match(as.character(JGI.full.tax$taxon_oid), JGItree.r$tip.label)), ]

JGI.domain <- JGI.full.tax$Domain
JGI.cyano <- JGI.full.tax$Phylum
JGI.group <- rep(NA, length(JGI.domain))

for (i in 1:length(JGI.cyano)){
  if (JGI.cyano[i] == "Cyanobacteria"){
    JGI.group[i] = "Cyanobacteria"
  } else {
    if (JGI.domain[i] == "Archaea"){
      JGI.group[i] = "Archaea"
    } else {
      JGI.group[i] = "Other Bacteria"
    }
  }
}

groupInfo <- split(JGItree.r$tip.label, JGI.group)
JGItree.r2 <- groupOTU(JGItree.r, groupInfo, group_name = "Cyano")
levels(attributes(JGItree.r2)$Cyano)  <- c("root", names(groupInfo))

groupInfo2 <- split(JGItree.r$tip.label, JGI.cyano)

subs <- subtrees(JGItree.r2)
archaea <- NA
for (i in 1:length(subs)){
  if (identical(subs[[i]]$tip.label, groupInfo$Archaea)){
    archaea <- JGItree.r2$edge[which(JGItree.r2$edge[, 2] == i 
                             +  length(JGItree$tip.label)), 1]
  }
}
...
for (i in 1:length(subs)){
  if (identical(subs[[i]]$tip.label, groupInfo$Cyanobacteria)){
    cyano <- JGItree.r2$edge[which(JGItree.r2$edge[, 2] == i 
                             +  length(JGItree$tip.label)), 1]
  }
}
...
proteo <- NA
for (i in 1:length(subs)){
  if (identical(subs[[i]]$tip.label, groupInfo2$Proteobacteria)){
    proteo <- JGItree.r2$edge[which(JGItree.r2$edge[, 2] == i 
                             +  length(JGItree$tip.label)), 1]
  } else {
    proteo <- 0
  }
...
JGI <- ggtree(JGItree.r2, aes(color = Cyano,
                             layout="rectangular"))
