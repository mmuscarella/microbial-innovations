...
```{r}
JGI.full <- read.tree("../data/JGI.first.tree")
JGI.unique <- read.tree("../data/JGI.first.unique.tree")
```
...
JGI.genes <- read.delim("../data/IMG.KEGG.txt", header = T)
```

## Match All Data
```{r}
# Find OIDs that match across all data
matching <- intersect(intersect(intersect(JGI.match$Genome, JGI.meta$taxon_oid), JGI.full$tip.label), JGI.genes$OID)

# Remove Tips in Tree Not Matching Across All Datasets
JGI.full.2 <- drop.tip(JGI.full, 
                       setdiff(JGI.full$tip.label, matching))

# Reorder just in case
JGI.full.2 <- reorder(JGI.full.2, order = "cladewise")

# Removing Any Taxonomy not Matching
JGI.full.tax <- JGI.meta[which(JGI.meta$IMG.Genome.ID %in% matching), ]
...
JGI.genes.2 <- JGI.genes[JGI.genes$OID %in% matching, ]

# Reorder to Match Tips
JGI.genes.2 <- JGI.genes.2[order(match(as.character(JGI.genes.2$OID), JGI.full.2$tip.label)), ]

# Check Match
tail(JGI.full.tax$taxon_oid)
tail(JGI.full.2$tip.label)
tail(JGI.genes.2$OID)

# Check Sizes
length(JGI.full.2$tip.label)
dim(JGI.full.tax)
dim(JGI.genes.2)
...
write.table(JGI.genes.2, "../data/IMG.KEGG.trimmed.txt",
            sep="\t")
```
...
## Show Plot
#```{r}
img <- readPNG("../figures/JGI_explore.png")
grid.raster(img)
```
...
```{r}
JGI.tree.r <- midpoint.root(JGI.full.2)

# Reorder Taxonomy and Genes to Match
JGI.full.tax <- JGI.full.tax[order(match(as.character(JGI.full.tax$taxon_oid), JGI.tree.r$tip.label)), ]
JGI.genes.2 <- JGI.genes.2[order(match(as.character(JGI.genes.2$OID), JGI.tree.r$tip.label)), ]

# Check Match
tail(JGI.full.tax$taxon_oid)
tail(JGI.tree.r$tip.label)
tail(JGI.genes.2$OID)
all.equal(JGI.tree.r$tip.label,
          as.character(JGI.full.tax$taxon_oid))
all.equal(JGI.tree.r$tip.label,
          as.character(JGI.genes.2$OID))
```

## Plot Rerooted Tree
```{r, results = "hide"}
png(filename="../figures/JGI_root.png",
    width = 1600, height = 1600, res = 96*2)

groupInfo <- split(JGI.tree.r$tip.label, JGI.full.tax$Domain)
JGI.tree.r2 <- groupOTU(JGI.tree.r, groupInfo, group_name = "Domain")
levels(attributes(JGI.tree.r2)$Domain)  <- c("root", names(groupInfo))

subs <- subtrees(JGI.tree.r2)
archaea <- NA
for (i in 1:length(subs)){
  if (identical(subs[[i]]$tip.label, groupInfo$Archaea)){
    archaea <- i
...
rt2 <- ggtree(JGI.tree.r2, aes(color = Domain),
              layout="rectangular")
rt2 + geom_point2(aes(subset=(node == 
                  (length(JGI.tree.r2$tip.label) + 1))),
                  size=3, shape=18, 
                  colour=ggplotColours(n = 4)[1], 
                  fill = "gray80") + 
      geom_text(aes(label = node), show.legend = FALSE,
...
## Show Plot
#```{r}
img <- readPNG("../figures/JGI_root.png")
grid.raster(img)
```

## Export Rooted Tree
```{r}
write.tree(JGI.tree.r, file = "../data/JGI.rooted.tre")
write.nexus(JGI.tree.r, file = "../data/JGI.rooted.nex")

# Find Sequences for Dating
tree.dist <- as.matrix(cophenetic.phylo(JGI.tree.r))
...
```{sh}
treePL ../config/JGI_treePL.config

# Output saved as JGI.dated.tree
```



# Explore Other Phylogenetic Groups
## JGI Exploratory Tree with Cyano Grouping
```{r, results = "hide"}
png(filename="../figures/JGI_Groups.png",
    width = 1600, height = 1600, res = 96*2)
...
# Creating Groupings
JGI.domain <- JGI.full.tax$Domain
JGI.phylum <- JGI.full.tax$Phylum
JGI.class <- JGI.full.tax$Class
JGI.group <- rep(NA, length(JGI.domain))

for (i in 1:length(JGI.phylum)){
  if (JGI.phylum[i] == "Cyanobacteria"){
    JGI.group[i] = "Cyanobacteria"
  }
  if (JGI.domain[i] == "Archaea"){
    JGI.group[i] = "Archaea"
  }
  if (JGI.class[i] == "Alphaproteobacteria"){
    JGI.group[i] = "Alpha"
  }
  if (JGI.class[i] == "Betaproteobacteria"){
  JGI.group[i] = "Beta"
  }
  if (JGI.class[i] == "Gammaproteobacteria"){
  JGI.group[i] = "Gamma"
  }
  if (JGI.phylum[i] == "Actinobacteria"){
  JGI.group[i] = "Actinobacteria"
  }
  if (JGI.phylum[i] == "Firmicutes"){
  JGI.group[i] = "Firmicutes"
  }
    if (JGI.phylum[i] == "Bacteroidetes"){
  JGI.group[i] = "Bacteroidetes"
  }
}

groupInfo <- split(JGI.tree.r$tip.label, JGI.group)
JGI.tree.r3 <- groupOTU(JGI.tree.r, groupInfo, group_name = "Groups")
levels(attributes(JGI.tree.r3)$Groups) <- c("root",
                 names(groupInfo))

# ID Substrees
subs <- subtrees(JGI.tree.r3)

# Find Nodes
archaea <- NA
for (i in 1:length(subs)){
  if (identical(subs[[i]]$tip.label, groupInfo$Archaea)){
    archaea <- JGI.tree.r3$edge[
      which(JGI.tree.r3$edge[, 2] == i
      +  length(JGI.tree.r3$tip.label)), 1]
  }
}
...
JGI <- ggtree(JGI.tree.r3, 
              aes(color = Groups, layout="rectangular"))

JGI + geom_text(aes(label = node), show.legend = FALSE,
            hjust=-0.1, vjust = -0.5, size = 2) + 
  geom_point2(aes(subset=(node == archaea)),
...
## Show Plot
#```{r}
img <- readPNG("../figures/JGI_Groups.png")
grid.raster(img)
```

# Import Ultrametric Tree
```{r, results = "hide"}
JGI.um <- read.tree("../data/JGI.dated.tree")

# Reorder just in case
JGI.um.2 <- reorder(JGI.um, order = "cladewise")

# Check Names Again
all.equal(JGI.um.2$tip.label,
          as.character(JGI.full.tax$taxon_oid))
all.equal(JGI.um.2$tip.label,
          as.character(JGI.genes.2$OID))

# Need to Reorder
JGI.full.tax <- JGI.full.tax[
  order(match(as.character(JGI.full.tax$taxon_oid), 
              JGI.um.2$tip.label)), ]
JGI.genes.2 <- JGI.genes.2[
  order(match(as.character(JGI.genes.2$OID), 
              JGI.um.2$tip.label)), ]
...
```{r}
png(filename="../figures/JGI_dated.png",
    width = 1600, height = 1600, res = 96*2)

# Create Grouping
JGI.domain <- JGI.full.tax$Domain
groupInfo <- split(JGI.um.2$tip.label, JGI.domain)
JGI.um.3 <- groupOTU(JGI.um.2, groupInfo, group_name = "Domain")
levels(attributes(JGI.um.3)$Domain) <- c("root",
                                         names(groupInfo))

um <- ggtree(JGI.um.3, aes(color = Domain),
             layout="rectangular")
...
## Show Plot
#```{r}
img <- readPNG("../figures/JGI_dated.png")
grid.raster(img)
```
...
```{r, results = "hide"}
png(filename="../figures/JGI_Branch_Comparision.png",
    width = 800, height = 800, res = 96*2)

# Are nodes the same?
all.equal(JGI.tree.r2$node.label, JGI.um.2$node.label)

reg <- JGI.tree.r2$edge.length
um <- JGI.um.2$edge.length
...
## Show Plot
#```{r}
img <- readPNG("../figures/JGI_Branch_Comparision.png")
grid.raster(img)
```
...
```{r, results = "hide"}
if (run.all == TRUE){
  png(filename="../figures/JGI_Tree_Topology.png",
      width = 1600, height = 1600, res = 96*2)
...
## Show Plot
#```{r}
img <- readPNG("../figures/JGI_Tree_Topology.png")
grid.raster(img)
```

## Calculate Local Minimums from Edge Lengths and Maximum from Polytomies
```{r}
den.dis <- density(log10(JGI.edges))
...
# Stopping Point
# Still needs work in Branch section
# Trait Innovation
```{r}
IMG.traits <- read.delim("../data/predicted_patyways.L3.txt",
                         skip = 1, row.names = 1)
dim(IMG.traits)
