...
trait.sum <- data.frame(matrix(NA, length(unique(IMG.Cons$trait)), 4))
colnames(trait.sum) <- c("FirstDate", "MedianDate", "LateDate", "NumEvol")

for (i in 1:length(unique(IMG.Cons$trait))){
  temp <- IMG.Cons[IMG.Cons$trait == i, ]
  first.evol <- max(temp$distance)
  med.evol <- median(temp$distance)
  last.evol <- min(temp$distance)
  n.evol <- dim(temp)[1]
  trait.sum[i, 1] <- first.evol
  trait.sum[i, 2] <- med.evol
  trait.sum[i, 3] <- last.evol
  trait.sum[i, 4] <- n.evol
}
